...
class NLSPNMetric(BaseMetric):
    ...
    def evaluate(self, sample, output, mode):
        with torch.no_grad():
            ...
            rmse = diff_sqr.sum() / (num_valid + 1e-7)
            rmse = torch.sqrt(rmse)

            mae = diff_abs.sum() / (num_valid + 1e-7)
            ...
            irmse = diff_inv_sqr.sum() / (num_valid + 1e-7)
            irmse = torch.sqrt(irmse)

            imae = diff_inv_abs.sum() / (num_valid + 1e-7)

            # Rel
            rel = diff_abs / (gt + 1e-7)
            rel = rel.sum() / (num_valid + 1e-7)
            ...
            del_1 = del_1.sum() / (num_valid + 1e-7)
            del_2 = del_2.sum() / (num_valid + 1e-7)
            del_3 = del_3.sum() / (num_valid + 1e-7)
